...
export async function generateMetadata({
  ...
  const url = `${SITE_URL}/saigoku/${num}`;
  const title = `第${temple.num}番 ${temple.name}（${temple.yomikata}）- 西国三十三所`;
  const desc = `西国三十三所第${temple.num}番札所・${temple.name}の参拝ガイド。${temple.shuha}。本尊: ${temple.honzon}。${temple.pref}。`;

  return {
    title,
    description: desc,
    keywords: [temple.name, "西国三十三所", temple.pref, temple.honzon, "巡礼", "御朱印"],
    alternates: { canonical: url },
    openGraph: {
      type: "article",
      title: `第${temple.num}番 ${temple.name} - 西国三十三所`,
      description: desc,
      url,
      siteName: SITE_NAME,
      locale: "ja_JP",
...
export default async function SaigokuDetailPage({ params }: PageProps) {
  const { num } = await params;
  const temple = getSaigokuTemple(Number(num));
  if (!temple) notFound();
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      <JsonLd
        data={{
          ...
          "@type": "BuddhistTemple",
          name: temple.name,
          alternateName: `西国三十三所 第${temple.num}番`,
      ...
      <JsonLd
        data={{
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 2, name: "西国三十三所", item: `${SITE_URL}/saigoku` },
            { "@type": "ListItem", position: 3, name: `第${temple.num}番 ${temple.name}` },
          ],
        }}
      />
      ...
      <div className="mt-8 space-y-8">
          ...
          <div className="p-6">
            ...
            <dl className="space-y-3 text-sm">
              <div className="flex gap-2">
                <dt className="w-20 shrink-0 font-medium text-[var(--color-muted)]">所在地</dt>
                <dd className="text-[var(--color-muted)]">{temple.address}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-20 shrink-0 font-medium text-[var(--color-muted)]">アクセス</dt>
                <dd className="text-[var(--color-muted)]">{temple.access}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-20 shrink-0 font-medium text-[var(--color-muted)]">参拝時間</dt>
                <dd className="text-[var(--color-muted)]">{temple.hours}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-20 shrink-0 font-medium text-[var(--color-muted)]">拝観料</dt>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-[var(--color-border)]">
          {prev ? (
            <Link href={`/saigoku/${prev.num}`} className="text-sm text-[var(--color-muted)] hover:text-[var(--color-gold)] transition-colors">
              ← 第{prev.num}番 {prev.name}
            </Link>
          ) : <span />}
          <Link href="/saigoku" className="text-sm text-[var(--color-muted)] hover:text-[var(--color-gold)] transition-colors">
            一覧に戻る
          </Link>
          {next ? (
            <Link href={`/saigoku/${next.num}`} className="text-sm text-[var(--color-muted)] hover:text-[var(--color-gold)] transition-colors">
