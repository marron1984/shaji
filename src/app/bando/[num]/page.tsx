...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!temple) return {};
  const url = `${SITE_URL}/bando/${num}`;
  return {
    title: `第${temple.num}番 ${temple.name}（${temple.yomikata}）- 坂東三十三観音`,
    description: `坂東三十三観音第${temple.num}番札所・${temple.name}の参拝ガイド。${temple.shuha}。本尊: ${temple.honzon}。${temple.pref}。`,
    keywords: [temple.name, "坂東三十三観音", temple.pref, temple.honzon, "巡礼"],
...
export default async function BandoDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <JsonLd data={{ "@context": "https://schema.org", "@type": "BuddhistTemple", name: temple.name, alternateName: `坂東三十三観音 第${temple.num}番`, description: temple.origin, address: { "@type": "PostalAddress", addressLocality: temple.pref, streetAddress: temple.address, addressCountry: "JP" }, url }} />
      <Breadcrumb items={[{ label: "ホーム", href: "/" }, { label: "坂東三十三観音", href: "/bando" }, { label: `第${temple.num}番 ${temple.name}` }]} />
      <div className="mt-6">
      ...
      <div className="mt-8 space-y-8">
        ...
        <div className="flex items-center justify-between pt-6 border-t border-[var(--color-border)]">
          {prev ? <Link href={`/bando/${prev.num}`} className="text-sm text-[var(--color-muted)] hover:text-[var(--color-gold)]">← 第{prev.num}番 {prev.name}</Link> : <span />}
          <Link href="/bando" className="text-sm text-[var(--color-muted)] hover:text-[var(--color-gold)]">一覧に戻る</Link>
          {next ? <Link href={`/bando/${next.num}`} className="text-sm text-[var(--color-muted)] hover:text-[var(--color-gold)]">第{next.num}番 {next.name} →</Link> : <span />}
        </div>
      </div>
    </div>
  );
